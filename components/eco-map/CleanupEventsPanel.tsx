...
export function CleanupEventsPanel({ refreshKey }: { refreshKey?: number }) {
  ...
  const [joinEvent, setJoinEvent] = useState<CleanupEvent | null>(null);
  ...
  // Leave (no form needed)
  const leave = async (e: CleanupEvent) => {
    if (!user) return;
    setEvents(es => es.map(x => x.id === e.id ? { ...x, joined: false, volunteerCount: x.volunteerCount - 1 } : x));
    try {
      await fetch("/api/cleanup/join", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ eventId: e.id, userId: user.id, join: false }),
      });
    } catch {
      load();
    }
    ...
    <>
      <Card>
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="ml-auto text-[10px] font-medium text-gd-text-muted">Fill the form to join → organizer notified by email</span>
        </div>
        {loading ? (
            ...
            {events.map(e => {
              ...
              return (
                <div key={e.id} className="rounded-xl border border-gd-border bg-gd-elevated/50 p-3.5 hover:bg-gd-elevated transition-colors">
                  ...
                  <button
                    onClick={() => (e.joined ? leave(e) : setJoinEvent(e))}
      ...
      <CleanupJoinModal
        ...
        event={joinEvent ? { id: joinEvent.id, title: joinEvent.title, date: joinEvent.date, rewardPoints: joinEvent.rewardPoints } : null}
      />
    </>
  );
}
