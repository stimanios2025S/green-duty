...
import { MapPin, AlertTriangle, CircleCheck, Circle, X } from "lucide-react";
import { hotspotReports } from "@/lib/mock-data";
import { Card } from "@/components/ui/Card";
import type { HotspotReport, SeverityLevel } from "@/types";
...
export function HotspotMap() {
  ...
  const filtered = filter === "all" ? hotspotReports : hotspotReports.filter(h => h.severity === filter);
          ...
          <div className="absolute inset-4">
            <div className="relative h-full w-full rounded-xl bg-gd-elevated border border-gd-border overflow-hidden bg-grid">
              <div className="absolute inset-0 p-4">
                {filtered.map((h) => {
                  const x = ((h.location.lng + 74.2) / 0.4) * 100;
                  const y = ((40.8 - h.location.lat) / 0.3) * 100;
                  return (
                    <button
                      key={h.id}
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gd-text-muted mt-0.5">{h.pollutionType.replace(/_/g, ' ')} · {h.location.address}</p>
            ...
            <div className="w-full max-w-lg rounded-2xl bg-gd-card border border-gd-border-soft p-6 shadow-2xl shadow-black/40" onClick={e => e.stopPropagation()}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gd-text-muted mt-0.5">{selected.location.address}</p>
                </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                <span className="rounded-full bg-gd-elevated px-3 py-1 text-xs font-medium text-gd-text-secondary border border-gd-border">{selected.pollutionType.replace(/_/g, ' ')}</span>
              ...
              <div className="flex items-center justify-between border-t border-gd-border pt-3">
                <span className="text-xs text-gd-text-muted">Reported by {selected.reporterName} · {selected.upvotes} upvotes</span>
