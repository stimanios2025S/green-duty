  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/instagro/users", {
        ...
        body: JSON.stringify({ userId: user.id, username, bio, emoji, gradient, name }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { setError(data.error || "Failed to save."); return; }
      updateUser({ name: name.trim(), bio, ...(data.user ? {} : {}) });
      setDone(true);
            ...
            <div className="space-y-5 overflow-y-auto p-5">
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="relative">
                  <InstaAvatar user={{ username, name, emoji, gradient }} size={84} />
                  <div className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full bg-gd-accent-500 ring-2 ring-gd-card">
                    <Camera className="h-3.5 w-3.5 text-gd-text-inverse" />
                  </div>
                </div>
                <div>
                  <p className="text-center text-xs font-medium text-gd-text-secondary">Avatar icon</p>
                  <div className="mt-1.5 flex flex-wrap justify-center gap-1.5">
                    {EMOJI_OPTIONS.slice(0, 10).map(e => (
                      <button key={e} onClick={() => setEmoji(e)} className={`flex h-8 w-8 items-center justify-center rounded-lg border text-base transition-all ${emoji === e ? "border-gd-accent-500/60 bg-gd-accent-500/10" : "border-gd-border bg-gd-elevated hover:border-gd-border-strong"}`}>{e}</button>
                    ))}
                  </div>
                </div>
