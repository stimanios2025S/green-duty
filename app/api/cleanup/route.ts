...
export async function POST(req: Request) {
  try {
    const { hotspotId, title, description, lat, lng, date, maxVolunteers, rewardPoints, organizerId } = await req.json();
    if (!title?.trim() || !organizerId) return NextResponse.json({ error: "Missing fields." }, { status: 400 });
    const d = await getDb();
    ...
    `).run(
      ...
      maxVolunteers || 25, rewardPoints || 100, organizerId,
