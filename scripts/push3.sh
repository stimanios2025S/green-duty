...
git add -A
git commit -m "debug: log jamendo count + status" 2>&1 | tail -2
git push 2>&1 | tail -2
...
echo "=== music test ==="
curl -s --max-time 40 "https://green-duty.vercel.app/api/music?q=meditation" | head -c 200
echo ""
echo "=== server logs ==="
vercel logs green-duty.vercel.app 2>&1 | grep -i "music" | tail -6
