...
git add -A
git commit -m "fix: no-store Jamendo fetch (stale cache was serving local fallback)" 2>&1 | tail -2
git push 2>&1 | tail -2
...
echo ""
echo "=== music (meditation) ==="
curl -s --max-time 40 "https://green-duty.vercel.app/api/music?q=meditation" | head -c 300
echo ""
echo "=== music (electronic) ==="
curl -s --max-time 40 "https://green-duty.vercel.app/api/music?genre=electronic" | head -c 200
echo ""
